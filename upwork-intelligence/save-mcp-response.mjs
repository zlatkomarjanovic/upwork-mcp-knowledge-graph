...
const [,, keyword, group, responsePath] = process.argv;
...
const err = resp.error || (resp.status && resp.status !== 'ok' ? resp.status : null);
fs.appendFileSync(log, JSON.stringify({ keyword, group, jobs, error: err }) + '\n');
