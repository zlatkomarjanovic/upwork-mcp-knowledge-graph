...
def collect_ordered(data: dict) -> list[dict]:
    """Pair find_jobs tool results in transcript order (args often omitted in exports)."""
    results = []
    for m in data.get("messages") or []:
        ...
        results.append(parse_result(raw))
    n = len(KEYWORDS)
    if len(results) >= n:
        return results[-n:]
    return results
...
def main() -> None:
    ...
    if len(by_kw) < len(KEYWORDS) // 2:
        ordered = collect_ordered(data)
        for i, spec in enumerate(KEYWORDS):
            if spec["keyword"] not in by_kw and i < len(ordered):
                by_kw[spec["keyword"]] = slim(ordered[i])
    existing_map = {}
    if SEARCH.exists():
