...
def proposals_from_job(job):
    if job.get("proposal_count") is not None:
        return job.get("proposal_count")
    tier = job.get("proposals_tier") or ""
    return {
        "Fewer than 5": 2,
        "5 to 10": 7,
        "10 to 15": 12,
...
def job_record(job, keyword, group):
    ...
    return {
        ...
        "proposals": proposals_from_job(job),
...
def main():
        ...
        if isinstance(data, list):
            for entry in data:
                if isinstance(entry, dict) and entry.get("keyword"):
                    batch_by_kw[entry["keyword"]] = entry
            continue
        kw = data.get("keyword")
        if kw:
            batch_by_kw[kw] = data
    ...
    for pname, pat in PLATFORM_KEYS.items():
        ...
        platform_stats[pname] = {
            ...
            "medianProposals": int(statistics.median([j["proposals"] for j in pjobs if j.get("proposals") is not None])) if pjobs else None,
