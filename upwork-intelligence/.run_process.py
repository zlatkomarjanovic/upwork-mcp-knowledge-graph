...
def main():
    ...
    for pname, pat in PLATFORM_KEYS.items():
        ...
        platform_stats[pname] = {
            ...
            "medianProposals": int(statistics.median([j["proposals"] for j in pjobs if j.get("proposals") is not None])) if [j for j in pjobs if j.get("proposals") is not None] else None,
