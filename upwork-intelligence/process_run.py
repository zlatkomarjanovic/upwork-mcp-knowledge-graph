...
def main() -> None:
    run_data_jsonl = ROOT / "run_data.jsonl"
    if run_data_jsonl.exists() and run_data_jsonl.stat().st_size > 0:
        import subprocess
        subprocess.run(
            [sys.executable, str(ROOT / "jsonl_to_mcp_raw.py"), str(run_data_jsonl)],
            check=False,
        )
    ...
    run_data_jsonl = ROOT / "run_data.jsonl"
    if run_data_jsonl.exists():
        for line in run_data_jsonl.read_text().splitlines():
            line = line.strip()
            if line:
