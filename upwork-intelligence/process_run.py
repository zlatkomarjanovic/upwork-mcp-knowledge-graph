...
def main() -> None:
    ...
    overlay_file = ROOT / "live_search_overlay.json"
    if overlay_file.exists():
        overlay = json.loads(overlay_file.read_text())
        for entry in paired:
            kw = entry["keyword"]
            if kw in overlay:
                entry["response"] = overlay[kw]
    state = load_state()
    ...
    if last_run_at and not first_run:
        try:
    ...
    live_jobs_file = ROOT / "live_jobs.jsonl"
    if live_jobs_file.exists():
        for line in live_jobs_file.read_text().splitlines():
            if not line.strip():
                continue
            j = json.loads(line)
