...
def normalize_proposals(value: Any) -> int | None:
    if value is None:
        return None
    if isinstance(value, (int, float)):
        return int(value)
    if isinstance(value, str):
        if value in TIER_MID:
            return TIER_MID[value]
        if value.isdigit():
            return int(value)
    return None
...
def load_jobs_index() -> dict[str, dict]:
    ...
    if JOBS_FILE.exists():
        for line in JOBS_FILE.read_text().splitlines():
            if line.strip():
                j = json.loads(line)
                j["proposals"] = normalize_proposals(j.get("proposals"))
...
def opportunity_score(jobs: list[dict]) -> int:
    ...
    for j in jobs:
        ...
        if j.get("type") == "fixed" and j.get("budgetFixed") and j["budgetFixed"] >= 1000:
            ...
            budget_boost = 0.6
        props = normalize_proposals(j.get("proposals")) or 20
...
def job_from_api(raw: dict, keyword: str, group: str) -> dict | None:
    ...
    proposals = normalize_proposals(raw.get("proposal_count"))
    if proposals is None and raw.get("proposals_tier"):
        proposals = TIER_MID.get(raw.get("proposals_tier"))
    return {
        "url": norm_url(url),
        "title": title,
...
def main() -> None:
    ...
    for spec in KEYWORDS:
        ...
        props = [p for j in matched if (p := normalize_proposals(j.get("proposals"))) is not None]
