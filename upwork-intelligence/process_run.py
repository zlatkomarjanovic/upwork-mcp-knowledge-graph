...
def load_jobs_index() -> dict[str, dict]:
    ...
    if JOBS_FILE.exists():
        for line in JOBS_FILE.read_text().splitlines():
            if line.strip():
                j = json.loads(line)
                if j.get("budgetFixed") is None and j.get("rateHourlyMid") is None:
                    fixed = rate = None
                    if j.get("type") == "fixed":
                        fixed = parse_money(j.get("budget"))
                    elif j.get("type") == "hourly":
                        rate = parse_money(j.get("budget"))
                    j["budgetFixed"] = fixed
                    j["rateHourlyMid"] = rate
                    if j.get("clientSpendNum") is None:
                        j["clientSpendNum"] = parse_client_spend(j.get("clientSpend"))
...
def job_from_api(raw: dict, keyword: str, group: str) -> dict | None:
    ...
    proposal_mid = {
        ...
        "20 to 50": 35,
        "50+": 60,
    }.get(prop_tier) if prop_tier else raw.get("proposal_count")
    return {
        "url": norm_url(url),
        "title": title,
...
def main() -> None:
    ...
    if state.get("lastRunAt") and not first_run:
        try:
            ...
            if gap_h > 1.25:
                window_hours = min(24.0, max(1.0, gap_h))
        except ValueError:
            pass
