...
def proposals_numeric(val) -> int | None:
    if val is None:
        return None
...
def opportunity_score(jobs: list[dict]) -> int:
    ...
    for j in jobs:
        ...
        if j.get("type") == "fixed" and j.get("budgetFixed") and j["budgetFixed"] >= 1000:
            ...
            budget_boost = 0.6
        props = proposals_numeric(j.get("proposals")) or 20
...
def job_from_api(raw: dict, keyword: str, group: str) -> dict | None:
    ...
    return {
        ...
        "proposals": raw.get("proposal_count") or raw.get("proposals_tier"),
...
def main() -> None:
    ...
    window_hours = 2.0 if first_run else 1.0
    if first_run and not jobs_index:
        window_hours = 2.0
    if state.get("lastRunAt") and not first_run:
        try:
            last = datetime.fromisoformat(state["lastRunAt"].replace("Z", "+00:00"))
            gap_h = (datetime.now(timezone.utc) - last).total_seconds() / 3600
            # Hourly cron: prefer 1h window; allow wider catch-up only after long downtime
            if gap_h > 6.0:
                window_hours = min(24.0, gap_h)
            elif gap_h > 1.25:
                window_hours = min(3.0, gap_h)
            else:
                window_hours = 1.0
        except ValueError:
            pass
    ...
    for spec in KEYWORDS:
        ...
        props = [proposals_numeric(j.get("proposals")) for j in matched]
